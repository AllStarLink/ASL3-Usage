    ...
    async def proc_reports(self, request):
        ...
        try:
            ...
            if c[2] == "basic" or c[2] == "basic/":
                ...
                r_txt =   "----------------------------------------------------------------------------------------------------------------------------------\n"
                r_txt += f" {'Collected':<20} | {'Node Cnt':<9}| {'Server Cnt':<10}| {'SimpleUSB Cnt':<14}| {'USBRadio Cnt':<13}| {'DAHDI Cnt':<10}| {'Voter Cnt':<10}| {'Diff Ast Ver':<13}| {'Diff ASL Ver':<13}\n"
                r_txt +=  "----------------------------------------------------------------------------------------------------------------------------------\n"

                for r in rows:
                    cit = r[10].strftime("%Y-%m-%d %H:%M:%S")
                    r_txt += f" {cit:>20} | {r[5]:>9}| {r[2]:>9} | {r[7]:>13} | {r[8]:>12} | {r[6]:>9} | {r[9]:>9} | {r[3]:>12} | {r[4]:>12}\n"
